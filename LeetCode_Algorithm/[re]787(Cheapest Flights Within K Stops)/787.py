...
class Solution:
    def findCheapestPrice(self, n: int, flights: List[List[int]], src: int, dst: int, k: int) -> int:
        ...
        queue = deque([(src, 0, 0)]); visited = [[False for _ in range(n+1)] for _ in range(n+1)]
        # print(visited)
        while queue:
            print(queue)
            node, price, temp_k = queue.popleft()
            if temp_k > k+1 or price >= ans:
                continue
            if node == dst:
                ans = min(ans, price); continue
            for (s, c) in hashMap[node]:
                if not visited[node][s]:
                    queue.append((s, price+c, temp_k+1))
                    visited[node][s] = True

        return -1 if sys.maxsize == ans else ans
